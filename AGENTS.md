...
- `install.zsh` installs Homebrew if needed, installs a few apps/tools, writes `~/.gitconfig`, symlinks `tmux.conf` to `~/.tmux.conf`, and makes `~/.zshrc` source this repo's `zshrc`.
